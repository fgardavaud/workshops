...
We need to be careful of using `accuracy()` since it can give "good" performance
by only predicting one way with imbalanced data:
...
lr_re_pred |>
  brier_class(truth = class, .pred_class_2)
```
...
lr_re_pred |> cal_plot_breaks(class, .pred_class_2)
```
...
```{r}
# Your code here!
lr_re_pred |> cal_plot_logistic(class, .pred_class_1)
lr_re_pred |> cal_plot_windowed(class, .pred_class_1)
```
...
⚠️ Remember that we're only repredicting the training set to demonstrate
overfitting - it is not a valid way to evaluate a model.
...
Fit `rf_wflow` to the training set and repredict it.
...
```{r}
augment(lr_fit, new_data = cls_train) |>
  roc_curve(truth = class, .pred_class_1) |>
  autoplot()
```
